...
void CPictureEx::SetAdStyle(bool bAdStyle)
{
	m_bAdStyle = bAdStyle;
}
...
BOOL CPictureEx::Load(HGLOBAL hGlobal, DWORD dwSize)
{
	...
	if (m_bAdaptTOClient)
	{
		m_PictureSize.cx=m_ClientRect.right-m_ClientRect.left;
		m_PictureSize.cy=m_ClientRect.bottom-m_ClientRect.top;
	}
...
BOOL CPictureEx::Load(LPCTSTR szFileName,BOOL bAdaptToClient,RECT ClientRect)
{
	ASSERT(szFileName);

	m_bAdaptTOClient=bAdaptToClient;
	if (bAdaptToClient)		
...
BOOL CPictureEx::Load(LPCTSTR szResourceName, LPCTSTR szResourceType,BOOL bAdaptToClient,RECT ClientRect)
{
		...
		//   3 - restore to previous
			...
			if (m_bAdaptTOClient)
			{
				if (m_arrFrames[m_nCurrFrame].m_pPicture->Render(m_hMemDC, 
					m_ClientRect.left,m_ClientRect.top,m_ClientRect.right-m_ClientRect.left,m_ClientRect.bottom-m_ClientRect.top,
				0, hmHeight, hmWidth, -hmHeight, NULL) == S_OK)
				{
					Invalidate(FALSE);
				}
			...
			else
			{
				...
				if (m_arrFrames[m_nCurrFrame].m_pPicture->Render(m_hMemDC, 
					m_arrFrames[m_nCurrFrame].m_frameOffset.cx, //显示范围的左上角X坐标
					m_arrFrames[m_nCurrFrame].m_frameOffset.cy, //显示范围的左上角Y坐标
					m_arrFrames[m_nCurrFrame].m_frameSize.cx, //显示范围的宽度
					m_arrFrames[m_nCurrFrame].m_frameSize.cy, //显示范围的高度
					0, hmHeight, hmWidth, -hmHeight, NULL) == S_OK)
				{
					Invalidate(FALSE);
				}
...
void CPictureEx::OnPaint() 
{
	...
	pDlg->m_picture.GetClientRect(&rect); //获得pictrue控件所在的矩形区域
	CDC *pDC = pDlg->m_picture.GetDC();//获得pictrue控件的DC
	height = m_PictureSize.cy;//得到图片高度
	width = m_PictureSize.cx;//得到图片宽度
	SetStretchBltMode(pDC->m_hDC, STRETCH_HALFTONE);
	if (width <= rect.Width() && height <= rect.Width()) //如果图片的尺寸小于图片控件尺寸，则不缩放显示
	{
		rect1 = CRect(rect.TopLeft(), CSize(width, height));
		//m_picture.Load(char2CString(filename));
		//::StretchBlt(pDC->m_hDC, rect1, SRCCOPY); //将图片画到Picture控件表示的矩形区域
		::StretchBlt(pDC->m_hDC, 0, 0, rect1.Width(), rect1.Height(),
			m_hMemDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy, SRCCOPY);
	}
...
{
	::SendMessage(::GetParent(m_hWnd), WM_COMMAND, MAKELONG(GetWindowLong(m_hWnd, GWL_ID), BN_CLICKED), (LPARAM)m_hWnd);
}
